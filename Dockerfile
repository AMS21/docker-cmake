...
RUN apk update && apk upgrade --no-cache && \
    ...
    CXXFLAGS="-w -O3 -flto -pipe -Wno-error=incompatible-pointer-types" ./bootstrap --parallel=$(nproc) && CXXFLAGS="-w -O3 -flto -pipe -Wno-error=incompatible-pointer-types" make -j $(nproc) && \
    make install && \
    cd .. && \
